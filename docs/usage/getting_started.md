...
So, after looking at the ``kiara module list`` output, it looks like the ``tabular.import_table_from_file`` module might be a good fit for us. *kiara* has the [``run``](../running_modules) sub-command, which is used to execute modules. If we
only provide a module name, and not any input, this command will tell us what it expects:

{{ cli("kiara", "run", "tabular.import_table_from_file") }}
...
{{ cli("kiara", "run", "tabular.import_table_from_file", "path=examples/data/journals/JournalNodes1902.csv", max_height=340) }}
...
{{ cli("kiara", "run", "--id", "getting_started_example", "--output", "format=silent", "--save", "tabular.import_table_from_file", "path=examples/data/journals/JournalNodes1902.csv") }}
...
And the *kiara* module we are going to use is called ``tabular.query.graphql``. Let's check again the parameters this module expects:

{{ cli("kiara", "run", "tabular.query.graphql") }}
...
```
➜ kiara run tabular.query.graphql table=value:getting_started_example.table query="$(cat examples/data/journals/query.graphql)"
...
modules again, and it looks like the ``network.graphs.import_network_graph`` one might do what we need.
...
{{ cli("kiara", "module", "explain-type", "network.graphs.import_network_graph", max_height=320) }}
...
{{ cli("kiara", "module", "explain-instance", "network.graphs.import_network_graph", max_height=320) }}
...
{{ cli("kiara", "run", "network.graphs.import_network_graph", "edges_path=examples/data/journals/JournalEdges1902.csv", "source_column=Source", "target_column=Target", "nodes_path=examples/data/journals/JournalNodes1902.csv", "nodes_table_index=Id", "--save", "--id", "generate_graph_from_csvs") }}
...
Now we might want to have a look at some of the intrinsic properties of our graph. For that, we will use the ``network.graphs.graph_properties`` module:

{{ cli("kiara", "run", "network.graphs.graph_properties", "graph=value:generate_graph_from_csvs.graph") }}

## Finding the shortest path

Another thing we can do is finding the shortes path between two nodes:

{{ cli("kiara", "run", "network.graphs.find_shortest_path", "graph=value:generate_graph_from_csvs.graph", "source_node=1", "target_node=2") }}
