...
from kiara.data_types import TYPE_PYTHON_CLS, DataType, DataTypeConfig, TYPE_CONFIG_CLS, DataTypeCharacteristics
from kiara.defaults import INVALID_HASH_MARKER, INVALID_SIZE_MARKER, KIARA_HASH_FUNCTION

SKALAR_CHARACTERISTICS = DataTypeCharacteristics(is_skalar=True, is_json_serializable=True)

class AnyType(DataType[object, DataTypeConfig], Generic[TYPE_PYTHON_CLS, TYPE_CONFIG_CLS]):
...
class KiaraModelValueType(AnyType[KiaraModel, DataTypeConfig], Generic[KIARA_MODEL_CLS, TYPE_CONFIG_CLS]):
    ...
    def _validate(self, data: KiaraModel) -> None:
        ...
        if not isinstance(data, self.__class__.python_class()):
            raise Exception(f"Invalid type '{type(data)}', must be: {self.__class__.python_class().__name__}, or subclass.")
