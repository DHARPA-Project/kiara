...
from kiara.kiara.persistence import DataStore, FilesystemDataStore
...
from kiara.value_types.type_mgmt import TypeMgmt
...
class Kiara(object):
    ...
    The `Kiara` object holds all information related to the current environment the user does works in. This includes:
      ...
      - available value value_types
      - available metadata schemas
      - available data items
      - available controller and processor value_types
    ...
    of its building blocks (modules, value value_types, etc.). So, you can't assume that, for example, a pipeline you create
    ...
    def __init__(
        ...
        self._data_store: DataStore = FilesystemDataStore(kiara=self)
    ...
    @property
    def value_type_classes(self) -> Mapping[str, Type[ValueType]]:
        return self.type_mgmt.value_type_classes
    ...
    def get_value_type(
        ...
        vt = self._type_mgmt_obj.retrieve_value_type(
            value_type=value_type, value_type_config=value_type_config
        )
        return vt
    ...
    def execute(self, manifest: Manifest, inputs: Dict[str, Any]) -> ValueSet:
