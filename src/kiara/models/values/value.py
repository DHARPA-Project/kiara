...
from kiara.defaults import (
    ...
    VALUES_CATEGORY_ID,
    SpecialValue, NO_MODULE_TYPE, VOID_KIARA_ID, KIARA_HASH_FUNCTION,
)
...
if TYPE_CHECKING:
#     from kiara.models.values.value_set import ValueSet
    from kiara.value_types import LoadConfig
...
class ValueDetails(KiaraModel):
    ...
    @property
    def value_type_config(self) -> Mapping[str, Any]:
    ...
    def __repr__(self):
        ...
        return f"{self.__class__.__name__}(id={self.value_id}, type={self.value_type_name}, status={self.value_status.value})"
    ...
    def create_renderable(self, **config: Any) -> RenderableType:
        ...
        for k in self.__fields__.keys():
            ...
            attr = getattr(self, k)
            if k == "pedigree_output_name" and attr == "__void__":
                continue

            if k == "pedigree" and attr == ORPHAN:
                v = "[i]-- external data --[/i]"
...
class Value(ValueDetails):
    ...
    def _needs_inititialization(self) -> bool:
    ...
    @property
    def data(self) -> Any:
        if self._needs_inititialization():
    ...
    def _retrieve_data(self) -> Any:
        ...
        retrieved = self._retrieve_value_data()
    ...
    def __repr__(self):
        ...
        return f"{self.__class__.__name__}(id={self.value_id}, type={self.value_type_name}, status={self.value_status.value}, initialized={self.is_initialized()})"

    @abc.abstractmethod
    def init_data(self, **data: Any) -> None:
        pass

    @abc.abstractmethod
    def is_initialized(self) -> bool:
        """Return whether this value is initialized with whatever seed this value value wrapper needs."""
        pass
...
class StaticValue(Value):
    ...
    def init_data(self, **data: Any):
        self._value_data = data["data"]

    def is_initialized(self) -> bool:
        result = not self.is_set or self._value_data != SpecialValue.NOT_SET
        return result

    def _retrieve_value_data(self):
        return self._value_data
