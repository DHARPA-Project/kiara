...
class PersistValueConfig(KiaraModuleConfig):
    ...
    source_type_config: Dict[str, Any] = Field(description="The value type config (if applicable).", default_factory=dict)
...
class SaveInternalModelModule(PersistValueModule):
    ...
    def data_type__internal_model(self, value: Value, target: str, base_name: str) -> LoadConfig:
        ...
        try:
            ...
            all_json = '{"data": ' + data_json + ', "python_class": ' + python_class_json + '}'
        except Exception as e:
            raise KiaraProcessingException(f"Can't serialize value of type '{value.value_schema.type}' to json: {e}.")
