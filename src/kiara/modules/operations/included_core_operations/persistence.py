...
from kiara.defaults import SERIALIZED_VALUE_TYPE_NAME, LOAD_CONFIG_VALUE_TYPE_NAME
...
from kiara.value_types.included_core_types.persistence import LoadConfigValueType

from kiara.modules import KiaraModule, ValueSetSchema


class PersistValueDetails(OperationDetails):
...
class PersistValueOperationType(OperationType[PersistValueDetails]):
    """An operation that takes a value, and saves it to disk and returns details about how to re-assemble the value (via a [kiara.value_types.included_core_types.persistence.LoadConfigValueType] object).
    ...
    def extract_details(self, module: "KiaraModule") -> Optional[PersistValueDetails]:
        ...
        for field_name, schema in module.outputs_schema.items():
            if schema.type != LOAD_CONFIG_VALUE_TYPE_NAME:
                continue
            else:
                if match != None:
                    log_message("ignore.operation", reason=f"More than one field of type '{LOAD_CONFIG_VALUE_TYPE_NAME}'", module_type=module.module_type_id)
                    continue
                else:
                    match = field_name
        ...
        for field_name, schema in module.inputs_schema.items():
            if field_name == schema.type:
                if input_field_match is not None:
                    ...
                    log_message("operation.ignore", module=module.module_type_id, reason=f"more than one input fields of type '{schema.type}'")
                    input_field_match = None
                    break
                else:
        ...
        load_config_type: LoadConfigValueType = self._kiara.type_mgmt.retrieve_value_type(value_type=value_schema.type, value_type_config=value_schema.type_config)  # type: ignore
