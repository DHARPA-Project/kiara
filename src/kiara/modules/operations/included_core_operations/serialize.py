...
from kiara.defaults import SERIALIZED_VALUE_TYPE_NAME
...
from kiara.modules.operations import OperationType, OperationDetails
from kiara.utils import log_message


class SerializeDetails(OperationDetails):
...
class SerializeOperationType(OperationType[SerializeDetails]):
    """An operation that takes a value, and serializes it into the format suitable to the [`serialized_value`][kiara.value_types.included_core_types.SeriailzedValue] value type.
    ...
    def extract_details(self, module: KiaraModule) -> Optional[SerializeDetails]:
        ...
        for field_name, schema in module.outputs_schema.items():
            if schema.type != SERIALIZED_VALUE_TYPE_NAME:
                continue
            else:
                if match != None:
                    log_message("ignore.operation", reason=f"More than one field of type '{SERIALIZED_VALUE_TYPE_NAME}'", module_type=module.module_type_id)
                    continue
                else:
                    match = field_name
        ...
        serialized_value_type: SerializedValue = self._kiara.type_mgmt.retrieve_value_type(value_type=value_schema.type, value_type_config=value_schema.type_config)  # type: ignore
    ...
    #     input_value_type = op_conf.value.value_type_name
    ...
    #
    #     input_value_type = inputs.value.value_type_name
    #
    #     op = self.find_serialzation_operation_for_type(input_value_type)
