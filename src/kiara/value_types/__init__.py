...
Anyway, the way it works (for now) is that *kiara* comes with a set of often used value_types (the standard set of: scalars,
...
In addition, packages that contain modules can implement their own, custom value_types, if suitable ones are not available in
core-*kiara*. Those can either be 'serialized/deserialized' into *kiara*-native value_types (which in turn will serialize them
...
from kiara.models.values.value import Value, ValuePedigree, StaticValue
...
class ValueTypeConfigSchema(BaseModel):
    """Base class that describes the configuration a [``ValueTypeOrm``][kiara.data.value_types.ValueTypeOrm] class accepts.

    This is stored in the ``_config_cls`` class attribute in each ``ValueTypeOrm`` class. By default,
    a ``ValueTypeOrm`` is not configurable, unless the ``_config_cls`` class attribute points to a sub-class of this class.
    """
...
TYPE_CONFIG_CLS = TypeVar("TYPE_CONFIG_CLS", bound=ValueTypeConfigSchema)


class ValueType(abc.ABC, Generic[TYPE_PYTHON_CLS, TYPE_CONFIG_CLS]):
    """Base class that all *kiara* value_types must inherit from.

    *kiara* value_types have 3 main responsibilities:
    ...
    @classmethod
    @abc.abstractmethod
    def value_type_config_class(cls) -> Type[TYPE_CONFIG_CLS]:
        pass

    @classmethod
    def _calculate_value_type_hash(
        cls, value_type_config: Union[Mapping[str, Any], ValueTypeConfigSchema]
    ) -> int:

        if isinstance(value_type_config, Mapping):
            value_type_config = cls.value_type_config_class()(**value_type_config)

        obj = {
            "type": cls._value_type_name,
            "type_config": value_type_config.config_hash,
        }
    ...
    def __init__(self, **type_config: Any):
        ...
        try:
            self._type_config: TYPE_CONFIG_CLS = self.__class__.value_type_config_class()(**type_config)  # type: ignore  # TODO: double-check this is only a mypy issue
        ...
        self._value_type_hash: Optional[int] = None

    @property
    def value_type_name(self) -> str:
        return self._value_type_name  # type: ignore

    @property
    def value_type_hash(self) -> int:
        if self._value_type_hash is None:
            self._value_type_hash = self.__class__._calculate_value_type_hash(
                self._type_config
            )
        return self._value_type_hash
    ...
    #                 value_type=self.__class__, value_data=load_config, exception=e
    #             )
    ...
    def assemble_static_value(self, value_id: uuid.UUID, data: Any, schema: ValueSchema, status: Union[ValueStatus, str],
                              value_hash: int, pedigree: ValuePedigree, kiara_id: uuid.UUID,
                              pedigree_output_name: str) -> Value:
        ...
        if status in [ValueStatus.SET, ValueStatus.DEFAULT]:
            ...
            try:
                data_new = self.parse_python_obj(data)
                ...
                raise KiaraValueException(
                    value_type=self.__class__, value_data=data, exception=e
                )
        else:
            size = 0
            value = StaticValue(
        ...
        value.init_data(data=data)

        return value

    def parse_python_obj(self, value: Any) -> TYPE_PYTHON_CLS:
...
#     def __init__(self, value_type_cls: typing.Type[ValueTypeOrm]):
#
#         self._value_type_cls: typing.Type[ValueTypeOrm] = value_type_cls
#
#     @property
#     def doc(self) -> str:
