...
import os
import shutil
from typing import Optional, Union, Any, List, Mapping, Dict, Type

import orjson.orjson
from deepdiff import DeepHash
from pydantic import PrivateAttr, Field, validator, BaseModel

from kiara.defaults import FILE_MODEL_CATEOGORY_ID, DEFAULT_EXCLUDE_FILES, FILE_BUNDLE_MODEL_CATEOGORY_ID, \
    KIARA_HASH_FUNCTION
...
from kiara.value_types import ValueTypeConfigSchema
from kiara.value_types.included_core_types import AnyType
...
class FileValueType(AnyType[FileModel, ValueTypeConfigSchema]):
    ...
    def calculate_size(self, value: FileModel) -> int:
        return value.size

    def calculate_hash(self, value: FileModel) -> int:
        return value.file_hash

    def pretty_print_as_renderables(
        self, value: "Value", print_config: Mapping[str, Any]
    ) -> Any:
...
class FileBundleValueType(AnyType[FileModel, ValueTypeConfigSchema]):
    ...
    def calculate_size(self, value: FileBundle) -> int:
        return value.size

    def calculate_hash(self, value: FileBundle) -> int:
